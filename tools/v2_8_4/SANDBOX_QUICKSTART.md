...
この手順は、独立再監査候補を新しい非本番Google Spreadsheetへ導入する利用者向けです。
このpackage生成時点では、実Google Workspace操作は実施していません。
最上位statusは`READY_FOR_INDEPENDENT_REAUDIT`です。Phase 8B GO/PASS、
Phase 8C GO、Pilot readyは宣言しません。
...
以降は同梱の
[Manual Acceptance Guide](MANUAL_ACCEPTANCE_GUIDE.md)をPart AからPart Lまで
順番に実行し、非機密の受入結果へPASS / FAIL / NOT EXECUTEDを記録します。
...
この手順の結果だけでPhase 8B GO/PASSまたはPhase 8C GOを宣言せず、
独立再監査の結果が確定するまでTEST_MODE=falseや個人実業務パイロットへ
進まないでください。
