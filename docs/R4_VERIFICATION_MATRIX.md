...
|---|---|---|---|
| R4-01 | Canonical source and release-tool topology | `remote_publication_consistency_test.js` | LOCAL_PENDING_FINAL |
| R4-02 | No root duplicate implementation/test/tool trees | `remote_publication_consistency_test.js` | LOCAL_PENDING_FINAL |
| R4-03 | Code/Schema/AI/Migration/Gate consistency | `remote_publication_consistency_test.js` | LOCAL_PENDING_FINAL |
| R4-04 | Historic A5/B5 preserved and verified | `Git ancestry audit` | VERIFIED_HISTORIC |
| R4-05 | Source/Release self-reference | `release verify scripts` | PENDING_FINAL_SOURCE |
| R4-06 | Release diff boundary | `remote_publication_consistency_test.js` | PENDING_RELEASE |
| R4-07 | 11 Sheets / hidden 5 | `static schema contract` | LOCAL_PENDING_FINAL |
| R4-08 | 50 Task columns | `static schema contract` | LOCAL_PENDING_FINAL |
| R4-09 | 21 Ledger columns | `static schema contract` | LOCAL_PENDING_FINAL |
...
| R4-27 | Diagnostics are read-only | `diagnostic source/test` | LOCAL_PENDING_FINAL |
...
Only final local test/static/release verification plus normal non-force GitHub
publication, final remote SHA resolution, and fresh-clone verification can
change the maximum status to `READY_FOR_INDEPENDENT_REAUDIT`. This matrix
does not declare Phase 8B GO/PASS, Phase 8C GO, production ready, or pilot
ready. Real Google Workspace evidence remains `NOT_EXECUTED`.
