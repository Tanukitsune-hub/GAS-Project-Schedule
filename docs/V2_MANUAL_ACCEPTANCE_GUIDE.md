...
> **Round 4 current acceptance baseline** — Code `2.8.5-prepilot`; Schema
> Version: `2.6`; AI Schema Version: `2.0`; Migration Version: `3`; 50 Task
> columns plus the 21-column protected hidden `Task Authority Ledger`.
> Automation default is `OFF`.  This guide remains evidence collection only:
> its highest status is `READY_FOR_INDEPENDENT_REAUDIT`, and it does not
> declare Phase 8B GO/PASS, Phase 8C GO, production ready, or pilot ready.
# TEST_MODE=true Sandbox 手動受入ガイド

- 対象version: `2.8.5-prepilot`
- Schema Version: `2.6`
...
1. `タスク一覧`の1行目が内部ID、2行目が日本語見出し、3行目以降がdata領域で
   あることを確認する。
2. `タスク一覧`が50列で、管理列が右側にあり、非表示・保護されること、および
   21列の`Task Authority Ledger`がhidden/protectedであることを確認する。
...
5. 専用negative-test Taskで、管理列1cell、利用者列と管理列のmixed paste、
   ...
   記録する。
6. 各管理列を含むeditがevent全体として拒否されることを確認する。authorityが
   VALIDな対象行はledgerのcommitted slotから全50列を復元し、authorityが欠落・
   複製・破損・曖昧な行は`QUARANTINED`または`UNRECOVERABLE`となることを確認する。
   後者の行から`authoritative_snapshot_json`やcell noteへfallbackしない。
...
- VALIDな行の復元後はTask ID、business fields、source/AI fields、physical/business
  version、Calendar metadata、durable intent、timestampおよびledger committed hashが
  一致する。隔離行はWorker、Review、Calendar処理から除外される。
...
4. さらに別のSpreadsheetでv1らしいSheet名またはheaderを用意し、Setupを
   実行する。
5. 専用cloneでのみ、Schema 2.6 Taskのauthority ledger recordを欠落・不正・
   duplicate・live row/hash不一致にしてSetupを実行し、live rowまたは
   `authoritative_snapshot_json`からauthorityを再生成せず、対象行が隔離されることを
   確認する。通常Sandboxでは実施せず`NOT EXECUTED`とする。
...
- Schema 2.6のauthority欠落・不正・task/schema不一致・hash driftをSetupがsilent
  rebaselineせず、対象行を隔離する。Migration 3は独立に証明されたSchema 2.5 legacy
  anchorだけを変換候補とし、current 2.6 rowのsnapshotやlive dataをtrust sourceにしない。
...
|---|---|
| Code 2.8.5-prepilot local +非機密Sandbox evidence | `READY_FOR_INDEPENDENT_REAUDIT` |
