...
/**
 * Build the Work 0041 versioned Phase 8B/8C packages and the derived
...
const codeVersion = '2.8.27-prepilot';
const phase8bPackage = `v${codeVersion}`;
const phase8cPackage = `v${codeVersion}-phase8c`;
const bundlePackage = 'work-0041-single-file-company-install';
...
function assertSourceContract(files) {
  ...
  for (const pattern of [
    /CODE_VERSION:\s*'2\.8\.27-prepilot'/,
...
function renderTemplate(templateName, replacements) {
  const templatePath = path.join(moduleRoot, 'tools', 'v2_8_27', templateName);
...
function sourceBeginMarker(name, byteLength, digest) {
  return `/* WORK_0041_SOURCE_BEGIN file=${name} bytes=${byteLength} sha256=${digest} */\n`;
}

function sourceEndMarker(name) {
  return `/* WORK_0041_SOURCE_END file=${name} */\n`;
}
...
function buildBundle(options) {
  ...
  const provenance = {
    schema: 'WORK_0041_SINGLE_FILE_INSTALL_V1',
    work_id: '0041',
    ...
    source_commit: options.sourceCommit,
    prepared_at: options.preparedAt,
    generated_by: 'implementation/GoogleSpreadsheet/tools/build_work_0041_release.js',
    ...
    separators: {
      begin_marker: '/* WORK_0041_SOURCE_BEGIN ... */',
      end_marker: '/* WORK_0041_SOURCE_END ... */',
      newline: '\n'
    }
  };
...
function buildRelease(options) {
  ...
  return {
    schema: 'WORK_0041_RELEASE_BUILD_RESULT_V1',
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else throw new Error('WORK_0041_UNKNOWN_ARGUMENT');
  }
  return result;
}
...
function main() {
  ...
  if (options.help) {
    process.stdout.write(
      'Usage: node tools/build_work_0041_release.js --source-commit SHA ' +
