...
const phase8bPath =
  'implementation/GoogleSpreadsheet/release/v2.8.15-prepilot';
const phase8cPath =
  'implementation/GoogleSpreadsheet/release/v2.8.15-prepilot-phase8c';
const releaseReportPath =
  'implementation/GoogleSpreadsheet/WORK_0028_RELEASE_IMPLEMENTATION_REPORT.md';
...
function checkRelease() {
  ...
  try {
    ...
    const outputs = [
      runPowerShell('verify_v2_8_15_release.ps1', [
        '-SourceCommit', contract.source_commit
      ], verificationModuleRoot),
      runPowerShell('verify_v2_8_15_phase8c_release.ps1', [
        '-SourceCommit', contract.source_commit
      ], verificationModuleRoot)
    ];
    return {
      command: 'v2.8.15 Phase 8B/8C package verifiers in committed LF checkout',
...
function checkReleaseLineage() {
  ...
  if (invalid.length) throw new Error('B15_SCOPE_INVALID');
  if (!changed.includes('CURRENT_CONTRACT.json') || !changed.includes(releaseReportPath) ||
    ...
    throw new Error('B15_REQUIRED_SCOPE_MISSING');
  }
  return {
    command: 'A15/B15 ancestry, release-only B15 scope, and source-stage absence',
