...
const phase8bPath =
  'implementation/GoogleSpreadsheet/release/v2.8.16-prepilot';
const phase8cPath =
  'implementation/GoogleSpreadsheet/release/v2.8.16-prepilot-phase8c';
const releaseReportPath =
  'implementation/GoogleSpreadsheet/WORK_0029_RELEASE_IMPLEMENTATION_REPORT.md';
...
function checkRelease() {
  ...
  try {
    ...
    const outputs = [
      runPowerShell('verify_v2_8_16_release.ps1', [
        '-SourceCommit', contract.source_commit
      ], verificationModuleRoot),
      runPowerShell('verify_v2_8_16_phase8c_release.ps1', [
        '-SourceCommit', contract.source_commit
      ], verificationModuleRoot)
    ];
    return {
      command: 'v2.8.16 Phase 8B/8C package verifiers in committed LF checkout',
...
function checkReleaseLineage() {
  ...
  if (invalid.length) throw new Error('B16_SCOPE_INVALID');
  if (!changed.includes('CURRENT_CONTRACT.json') || !changed.includes(releaseReportPath) ||
    ...
    throw new Error('B16_REQUIRED_SCOPE_MISSING');
  }
  return {
    command: 'A16/B16 ancestry, release-only B16 scope, and source-stage absence',
