...
const phase8bPath =
  'implementation/GoogleSpreadsheet/release/v2.8.19-prepilot';
const phase8cPath =
  'implementation/GoogleSpreadsheet/release/v2.8.19-prepilot-phase8c';
const releaseReportPath =
  'implementation/GoogleSpreadsheet/WORK_0032_RELEASE_IMPLEMENTATION_REPORT.md';
...
function checkRelease() {
  ...
  try {
    ...
    const outputs = [
      runPowerShell('verify_v2_8_19_release.ps1', [
        '-SourceCommit', contract.source_commit
      ], verificationModuleRoot),
      runPowerShell('verify_v2_8_19_phase8c_release.ps1', [
        '-SourceCommit', contract.source_commit
      ], verificationModuleRoot)
    ];
    return {
      command: 'v2.8.19 Phase 8B/8C package verifiers in committed LF checkout',
...
function checkReleaseLineage() {
  ...
  if (invalid.length) throw new Error('B19_SCOPE_INVALID');
  if (!changed.includes('CURRENT_CONTRACT.json') || !changed.includes(releaseReportPath) ||
    ...
    throw new Error('B19_REQUIRED_SCOPE_MISSING');
  }
  return {
    command: 'A19/B19 ancestry, release-only B19 scope, and source-stage absence',
