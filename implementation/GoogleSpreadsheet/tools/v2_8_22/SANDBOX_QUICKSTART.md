# Work 0037 Sandbox Quickstart - 2.8.22-prepilot
...
2. Verify the actual Automation state is consistently OFF with zero scheduled
   and clock triggers.
3. Verify the label-gated Inbox query and manual-label guard and the one-candidate
...
5. If a later Work authorizes validation, use only the exact fictional
   `手動/取込` label-gated candidate and the no-argument readiness entrypoint;
