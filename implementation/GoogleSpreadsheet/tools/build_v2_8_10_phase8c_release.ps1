...
$releaseVersion = 'v2.8.10-prepilot-phase8c'
$expectedCodeVersion = '2.8.10-prepilot'
...
$expectedModuleContractId = 'WORK_OS_V2_S90_CONTRACT_2_8_10'
...
function Assert-ExactSourceCheckout {
  ...
  if ($headCommit.ToLowerInvariant() -ne $resolvedSourceCommit.ToLowerInvariant()) {
    throw (
      ...
      'Create/checkout Source A10 first; package generation stopped.'
    )
  }
}
...
[pscustomobject]@{
  ...
  HighestLocalStatus = 'PHASE8B_SANDBOX_NO_GO_DASHBOARD_WRITE_VISIBILITY'
} | Format-List
