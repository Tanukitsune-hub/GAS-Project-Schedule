...
/**
 * Verify Work 0039 versioned packages and the derived two-paste bundle.
...
const builder = require('./build_work_0039_release');
...
function verifyPackage(options) {
  ...
  for (const text of [manualGuide, quickstart]) {
    if (!text.includes('2.8.26-prepilot') ||
        !text.includes(gate) ||
...
function markerBegin(name, byteLength, digest) {
  return Buffer.from(
    `/* WORK_0039_SOURCE_BEGIN file=${name} bytes=${byteLength} sha256=${digest} */\n`,
    'utf8'
  );
}

function markerEnd(name) {
  return Buffer.from(`/* WORK_0039_SOURCE_END file=${name} */\n`, 'utf8');
}
...
function verifyBundle(sourceCommit, preparedAt) {
  ...
  builder.phase8cSourceOrder.forEach((name, index) => {
    ...
    if (codeText.split(`/* WORK_0039_SOURCE_BEGIN file=${name} `).length - 1 !== 1 ||
        codeText.split(`/* WORK_0039_SOURCE_END file=${name} */`).length - 1 !== 1) {
  ...
  new vm.Script(code.toString('utf8'), { filename: 'Work0039-Code.gs' });
...
function verifyReproducibility(sourceCommit, preparedAt) {
  const temporaryRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), 'work-0039-release-verify-')
  );
  try {
...
function readContract() {
  ...
  if (value.schema !== 'WORK_OS_CURRENT_CONTRACT_V1' ||
      value.branch !== 'codex/0039-openai-provider-selection' ||
...
function verifyRelease(options = {}) {
  ...
  return {
    schema: 'WORK_0039_RELEASE_VERIFICATION_RESULT_V1',
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else throw new Error('WORK_0039_VERIFY_UNKNOWN_ARGUMENT');
  }
  return result;
}
...
function main() {
  ...
  if (options.help) {
    process.stdout.write(
      'Usage: node tools/verify_work_0039_release.js ' +
