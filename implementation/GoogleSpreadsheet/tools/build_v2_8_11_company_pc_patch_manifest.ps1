...
param(
  ...
  [ValidatePattern('^[0-9a-f]{40}$')]
  [string]$OldFixedRef = '927d8567bce64461840cc6f72fbae0c1e636a8e6',
...
$oldVersion = 'v2.8.10-prepilot'
$newVersion = 'v2.8.11-prepilot'
$oldPayloadRoot = 'implementation/GoogleSpreadsheet/release/v2.8.10-prepilot/apps-script'
$newPayloadRoot = 'implementation/GoogleSpreadsheet/release/v2.8.11-prepilot/apps-script'
...
if (-not $OutputDirectory) { $OutputDirectory = Join-Path $moduleRoot 'transfer/v2.8.11-prepilot' }
...
$safeCompletedStages = @(
  ...
  'S90_QUICK_DIAGNOSTIC', 'S99_COMPLETE'
)
$safeIncompleteStages = @()
...
$expectedConfig = [ordered]@{code_version='2.8.11-prepilot';schema_version='2.6';ai_schema_version='2.0';migration_version='3';test_mode=$true;automation_enabled=$false}
$manifest = [ordered]@{
...
$mdLines = @(
  ...
  '## Post-update configuration checks','','- CODE_VERSION=2.8.11-prepilot','- SCHEMA_VERSION=2.6','- AI_SCHEMA_VERSION=2.0','- MIGRATION_VERSION=3','- TEST_MODE=true','- AUTOMATION_ENABLED=false','',
  '## Safe completed-Sandbox retransfer','', 'Treat the Sandbox as already complete through S99. Do not run Setup or resume any Setup stage. With separately granted execution authority only, reload the replacement files and run T1-01 Quick Diagnostic once. Record only the bounded summary fields and then stop.','Do not duplicate, delete, overwrite, or manually repair Gmail labels, the dedicated Calendar, Properties, the owner edit trigger, Task Authority Ledger, Task data, or Dashboard seed. Automation stays OFF and no five-minute trigger is created. A version-property mismatch must remain an explicit WARN; do not reconcile it.','',
