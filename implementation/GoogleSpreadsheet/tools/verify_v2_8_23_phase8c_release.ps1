param(
  [string]$ReleaseVersion = 'v2.8.23-prepilot-phase8c',
...
$expectedConfig = Convert-AuditedPhase8CConfig -SourceConfig $sourceConfig
...
if ($payloadConfig -ne $expectedConfig) {
  throw 'Phase 8C Config is not the exact audited production configuration transform.'
}
foreach ($pattern in @(
    ...
    'TEST_MODE:\s*false',
    'EXTERNAL_AI_ENABLED:\s*true',
    'EXTERNAL_AI_OPERATOR_APPROVED:\s*true',
    'EXTERNAL_AI_DATA_POLICY_APPROVED:\s*true',
    'EXTERNAL_AI_CREDENTIAL_STORAGE_APPROVED:\s*true',
    'EXTERNAL_AI_AUTH_CONFIGURED:\s*true',
    'EXTERNAL_AI_COMPANY_APPROVED:\s*false',
...
foreach ($literal in @(
  ...
  if (-not $payloadConfig.Contains($literal)) {
...
[pscustomobject]@{
  ...
  SourceParityExceptAuditedProductionConfigTransform = 'PASS'
  Checksums = 'PASS'
