...
/**
 * Work 0033-only one-use Gemini candidate placement lane.
 *
 * The lane reuses the consumed, locally bound Work 0010 personal-synthetic
 * target only after proving the completed Work 0032 placement on that binding.
...
const sourceRootFromRepository = path.relative(repositoryRoot, sourceRoot)
  .split(path.sep).join('/');
const workspaceName = '.clasp-work-0033';
const pullWorkspaceName = '.clasp-pull-verify-work-0033';
...
const executionStateFileName = 'work-0033-execution-state.json';
const executionStatePath = path.join(workspaceRoot, executionStateFileName);
const operationLockPath = path.join(workspaceRoot, 'work-0033-operation.lock');
...
function assertExactBranchCleanAndPublished(expectedHead) {
  ...
  if (!isExactBranch(branch)) fail('WORK_0033_EXACT_BRANCH_REQUIRED');
  if (String(git([
  ...
  if (ancestry.error || ancestry.status !== 0) {
    fail('WORK_0033_INSTRUCTION_ANCESTRY_INVALID');
  }
  const remote = String(git(['rev-parse', `origin/${exactBranch}`])).trim();
  if (remote !== head) fail('WORK_0033_REPAIR_HEAD_NOT_PUBLISHED');
  if (expectedHead && expectedHead !== head) {
    fail('WORK_0033_REPAIR_HEAD_CHANGED');
  }
  return head;
}
...
function acquireOperationLock(command, lockPath = operationLockPath) {
  ...
  try {
    ...
    fail('WORK_0033_OPERATION_ALREADY_RUNNING');
  }
  return () => {
    try {
...
function assertIdentifierNotTracked(identifier) {
  ...
  if (result.status === 0) fail('WORK_0033_TARGET_IDENTIFIER_IS_TRACKED');
  if (result.status !== 1) fail('WORK_0033_TRACKED_IDENTIFIER_SCAN_FAILED');
}
...
function assertExistingBindingObjects(config, target, state) {
  try {
    ...
    if (error instanceof ClaspGateError) {
      fail('WORK_0033_EXISTING_TARGET_BINDING_INVALID');
    }
    throw error;
  }
  ...
  if (!valid) fail('WORK_0033_EXISTING_TARGET_BINDING_INVALID');
  return { config, target, state };
}

function loadExistingBinding() {
  const binding = assertExistingBindingObjects(
    readJson(sourceConfigPath, 'WORK_0033_EXISTING_TARGET_CONFIG_MISSING'),
    readJson(sourceTargetPath, 'WORK_0033_EXISTING_TARGET_METADATA_MISSING'),
    readJson(sourceStatePath, 'WORK_0033_EXISTING_TARGET_STATE_MISSING')
  );
  ...
  var previousPlacement = readJson(
    ...
    'WORK_0033_PREVIOUS_PLACEMENT_STATE_MISSING'
  );
...
function assertPreviousPlacement(binding, previousPlacement) {
  var previousValid = previousPlacement &&
    previousPlacement.schema ===
      'WORK_OS_GEMINI_RUNTIME_DIAGNOSTICS_PLACEMENT_V1' &&
    previousPlacement.work_id === '0032' &&
    previousPlacement.source_binding_work_id === '0010' &&
    previousPlacement.previous_placement_work_id === '0031' &&
  ...
  if (!previousValid) fail('WORK_0033_PREVIOUS_PLACEMENT_STATE_INVALID');
...
function assertPayloadInventory(inventory) {
  if (!inventory || inventory.schema !== 'WORK_OS_LOCAL_CLASP_PAYLOAD_V1' ||
    ...
    fail('WORK_0033_CANONICAL_PAYLOAD_MISMATCH');
  }
  return inventory;
}
...
function assertStateBase(state) {
  const valid = state &&
    state.schema === 'WORK_OS_GEMINI_SCHEMA_COMPATIBILITY_PLACEMENT_V1' &&
    state.work_id === '0033' && state.source_binding_work_id === '0010' &&
    state.previous_placement_work_id === '0032' &&
  ...
  if (!valid) fail('WORK_0033_EXECUTION_STATE_INVALID');
}
...
function nextAttemptState(state, command) {
  ...
  if (command === 'push') {
    if (state.phase !== 'STAGED' || state.push_attempt_count !== 0 ||
        state.pull_attempt_count !== 0) fail('WORK_0033_PUSH_ALREADY_ATTEMPTED');
  ...
  if (command === 'pull-verify') {
    if (state.phase !== 'PUSH_PASS' || state.push_attempt_count !== 1 ||
        state.pull_attempt_count !== 0) fail('WORK_0033_PULL_ALREADY_ATTEMPTED');
    next.pull_attempt_count = 1;
    next.phase = 'PULL_ATTEMPT_STARTED';
    return next;
  }
  fail('WORK_0033_REMOTE_COMMAND_INVALID');
}

function stagePayload() {
  if (process.env.GAS_WORK_0033_CI_CONFIRMED !== 'true') {
    fail('WORK_0033_REPAIR_CI_CONFIRMATION_REQUIRED');
  }
  const head = assertExactBranchCleanAndPublished();
  if (fs.existsSync(workspaceRoot) || fs.existsSync(pullRoot)) {
    fail('WORK_0033_LOCAL_STATE_ALREADY_EXISTS');
  }
  ...
  if (JSON.stringify(staged) !== JSON.stringify(committed)) {
    fail('WORK_0033_STAGED_PAYLOAD_SOURCE_SKEW');
  }
...
function assertStagedPayload() {
  if (!fs.existsSync(payloadRoot) || !fs.existsSync(inventoryPath) ||
    ...
    fail('WORK_0033_STAGED_PAYLOAD_MISSING');
  }
  const state = readJson(executionStatePath, 'WORK_0033_EXECUTION_STATE_INVALID');
  assertStateBase(state);
  assertExactBranchCleanAndPublished(state.repair_head);
  const binding = loadExistingBinding();
  const config = readJson(configPath, 'WORK_0033_TARGET_CONFIG_MISSING');
  if (config.scriptId !== binding.config.scriptId ||
      state.target_fingerprint !== binding.state.target_fingerprint) {
    fail('WORK_0033_EXISTING_TARGET_CHANGED');
  }
  try {
  ...
  assertExactPayloadDirectory(
    payloadRoot, 'WORK_0033_STAGED_PAYLOAD_UNEXPECTED_CONTENT'
  );
  const names = canonicalPayloadNames();
  const saved = assertPayloadInventory(readJson(
    inventoryPath, 'WORK_0033_STAGED_PAYLOAD_INVENTORY_INVALID'
  ));
  const staged = assertPayloadInventory(inventoryFor(payloadRoot, names));
  const committed = assertPayloadInventory(inventoryForCommittedPayload(names));
  if (JSON.stringify(saved) !== JSON.stringify(staged) ||
    ...
    fail('WORK_0033_STAGED_PAYLOAD_MISMATCH');
  }
...
function isolatedNativeSelection() {
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'work-0033-native-'));
  try {
...
async function existingAuthStatus() {
  ...
  try {
    ...
    return {
      lane: 'work_0033_existing_target_placement',
      command: 'auth-status',
      status: 'PASS',
...
function pushPayload() {
  if (process.env.GAS_WORK_0033_PUSH_ALLOWED !== 'true' ||
      process.env.GAS_DEV_CLASP_ALLOWED !== 'true') {
    fail('WORK_0033_PUSH_OPT_IN_REQUIRED');
  }
  const staged = assertStagedPayload();
  ...
  writeJsonAtomic(executionStatePath, started);
  return {
    lane: 'work_0033_existing_target_placement', command: 'push', status: 'PASS',
...
function preparePullWorkspace(config) {
  if (fs.existsSync(pullRoot)) fail('WORK_0033_PULL_WORKSPACE_ALREADY_EXISTS');
...
function pullVerify() {
  if (process.env.GAS_WORK_0033_PULL_ALLOWED !== 'true' ||
      process.env.GAS_DEV_CLASP_ALLOWED !== 'true') {
    fail('WORK_0033_PULL_OPT_IN_REQUIRED');
  }
  const staged = assertStagedPayload();
  ...
  if (result.exit_code !== 0) fail('WORK_0033_CLASP_PULL_FAILED');
  const pulledRoot = path.join(pullRoot, 'payload');
  assertExactPayloadDirectory(
    pulledRoot, 'WORK_0033_PULLBACK_UNEXPECTED_CONTENT'
  );
  ...
  if (JSON.stringify(pulled) !== JSON.stringify(staged.inventory)) {
    fail('WORK_0033_PULLBACK_PARITY_FAILED');
  }
  Object.assign(started, {
...
async function main() {
  ...
  try {
    ...
    if (command === 'auth-status') safeWrite(await existingAuthStatus());
      ...
      lane: 'work_0033_existing_target_placement', command, status: 'PASS',
      google_operation: 'NOT_EXECUTED'
    }, stagePayload()));
    ...
    else fail('UNKNOWN_WORK_0033_COMMAND');
  } catch (error) {
    const code = error && error.code || 'WORK_0033_OPERATION_FAILED';
    safeWrite({
      lane: 'work_0033_existing_target_placement', command, status: code,
      sensitive_output: 'SUPPRESSED', attempts: safeAttemptEvidence(),
      message: code
    });
    process.exitCode = 2;
