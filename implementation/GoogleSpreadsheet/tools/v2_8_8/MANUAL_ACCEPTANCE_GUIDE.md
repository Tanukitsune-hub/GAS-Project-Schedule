...
# 2.8.8-prepilot Manual Acceptance Guide
...
| Highest status allowed here | `PHASE8B_SANDBOX_NO_GO_DASHBOARD_SURFACE` |
...
bounded, resumable, and isolated from current-schema silent repair.
...
3. Confirm the Source A8 commit contains source/tests/tools/canonical docs/
   changelog/visualization/design/migration only.
4. Confirm the Release B8 commit contains only the two release packages and
   the Phase 8B Dashboard surface real-runtime implementation report.
...
| Overall maximum status | `PHASE8B_SANDBOX_NO_GO_DASHBOARD_SURFACE` |  |
