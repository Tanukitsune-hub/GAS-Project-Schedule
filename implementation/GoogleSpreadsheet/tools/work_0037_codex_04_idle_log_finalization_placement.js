...
/**
 * Exactly-once Work 0037 CODEX-04 Phase 8C correction lane.
 *
...
const instructionHead = '71b0ea873b179cd155df958d69d609daace454f9';
const phase8cReleaseRelativeRoot =
  'implementation/GoogleSpreadsheet/release/v2.8.25-prepilot-phase8c/apps-script';
const phase8cSchema =
  'WORK_OS_CODEX_04_IDLE_LOG_FINALIZATION_REPLACEMENT_V1';
const laneName = 'work_0037_codex_04_idle_log_finalization_placement';
const ciConfirmationRequiredEnv =
  'GAS_WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_CI_CONFIRMED';
const pushAllowedEnv = 'GAS_WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_PUSH_ALLOWED';
const pullAllowedEnv = 'GAS_WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_PULL_ALLOWED';
const workspaceName = '.clasp-work-0037-codex-04-idle-log-finalization';
...
const executionStatePath = path.join(
  workspaceRoot,
  'work-0037-codex-04-idle-log-finalization-execution-state.json'
);
const operationLockPath = path.join(
  workspaceRoot,
  'work-0037-codex-04-idle-log-finalization-operation.lock'
);
...
const historicalAutomaticInboxStatePath = path.join(
  moduleRoot,
  '.clasp-work-0037-automatic-inbox-shadow-pilot',
  'work-0037-automatic-inbox-shadow-pilot-execution-state.json'
);
const historicalCodex03StatePath = path.join(
  moduleRoot,
  '.clasp-work-0037-codex-03-operational-log-hardening',
  'work-0037-codex-03-operational-log-hardening-execution-state.json'
);
...
function acquireOperationLock(command) {
  ...
  try {
    ...
    fail('WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_OPERATION_ALREADY_RUNNING');
  }
  return () => {
...
function assertBranchCleanPublished() {
  if (String(git(['branch', '--show-current'])).trim() !== exactBranch) {
    fail('WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_EXACT_BRANCH_REQUIRED');
  }
  if (String(git([
  ...
  if (ancestor.error || ancestor.status !== 0) {
    fail('WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_INSTRUCTION_ANCESTRY_INVALID');
  }
  if (String(git(['rev-parse', `origin/${exactBranch}`])).trim() !== head) {
    fail('WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_HEAD_NOT_PUBLISHED');
  }
  return head;
}
...
function phase8cPayloadNames() {
  ...
  if (names.length !== 23 ||
    ...
    fail('WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_PHASE8C_INVENTORY_INVALID');
  }
  return names;
}
...
function committedBuffer(name) {
  ...
  if (result.error || result.status !== 0) {
    fail('WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_RELEASE_READ_FAILED');
  }
  return result.stdout;
}
...
function assertAutomationOffEvidence() {
  if (process.env.GAS_WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_AUTOMATION_OFF_CONFIRMED !== 'true') {
...
function assertHistoricalStateUnchangedEvidence(binding) {
  const historical = [
    {
      path: historicalWork0037StatePath,
      schema: 'WORK_OS_PERSONAL_SHADOW_PILOT_REPLACEMENT_V1',
      missing: 'WORK_0037_HISTORICAL_LABEL_GATED_STATE_MISSING',
      invalid: 'WORK_0037_HISTORICAL_LABEL_GATED_STATE_INVALID'
    },
    {
      path: historicalAutomaticInboxStatePath,
      schema: 'WORK_OS_AUTOMATIC_INBOX_SHADOW_PILOT_REPLACEMENT_V1',
      missing: 'WORK_0037_HISTORICAL_AUTOMATIC_INBOX_STATE_MISSING',
      invalid: 'WORK_0037_HISTORICAL_AUTOMATIC_INBOX_STATE_INVALID'
    },
    {
      path: historicalCodex03StatePath,
      ...
      missing: 'WORK_0037_HISTORICAL_CODEX_03_STATE_MISSING',
      invalid: 'WORK_0037_HISTORICAL_CODEX_03_STATE_INVALID'
    }
  ];
  historical.forEach((entry) => {
    const state = readJson(entry.path, entry.missing);
    if (state.schema !== entry.schema || state.work_id !== '0037' ||
        state.phase !== 'PULL_PARITY_PASS' ||
        state.push_attempt_count !== 1 || state.pull_attempt_count !== 1 ||
        state.pull_parity !== 'PASS' ||
        (entry.payload_path && state.payload_path !== entry.payload_path) ||
        state.target_fingerprint !== binding.state.target_fingerprint) {
      fail(entry.invalid);
    }
  });
  return {
    historical_state: 'PRESERVED_AND_NOT_REUSED_AS_AUTHORITY',
    historical_state_count: historical.length
  };
}
...
function initialState(head, inventory, binding) {
  return {
    ...
    historical_placement_work_id: '0037-codex-03-operational-log-hardening',
...
function assertState(state) {
  if (!state || state.schema !== phase8cSchema || state.work_id !== '0037' ||
      ...
      state.historical_placement_work_id !==
        '0037-codex-03-operational-log-hardening' ||
    ...
    fail('WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_EXECUTION_STATE_INVALID');
  }
}

function stagedPayload() {
  if (process.env[ciConfirmationRequiredEnv] !== 'true') {
    fail('WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_CI_CONFIRMATION_REQUIRED');
  }
  ...
  if (fs.existsSync(workspaceRoot) || fs.existsSync(pullRoot)) {
    fail('WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_LOCAL_STATE_ALREADY_EXISTS');
  }
  ...
  const committed = assertInventory(
    ...
    'WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_COMMITTED_INVENTORY_INVALID'
  );
  fs.mkdirSync(payloadRoot, { recursive: true });
  for (const name of names) {
    fs.writeFileSync(path.join(payloadRoot, name), committedBuffer(name));
  }
  assertExactPayloadDirectory(payloadRoot, 'WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_PAYLOAD_INVALID', names);
  ...
  const staged = assertInventory(
    ...
    'WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_STAGED_INVENTORY_INVALID'
  );
  if (JSON.stringify(staged) !== JSON.stringify(committed)) {
    fail('WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_SOURCE_SKEW');
  }
...
function assertStaged() {
  if (!fs.existsSync(workspaceRoot) || !fs.existsSync(payloadRoot) ||
    ...
    fail('WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_STAGED_STATE_MISSING');
  }
  const state = readJson(executionStatePath, 'WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_EXECUTION_STATE_INVALID');
  assertState(state);
  const head = assertBranchCleanPublished();
  if (state.repair_head !== head) fail('WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_STALE_HEAD');
  const binding = loadBinding();
  assertHistoricalStateUnchangedEvidence(binding);
  const names = phase8cPayloadNames();
  const saved = assertInventory(
    readJson(inventoryPath, 'WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_INVENTORY_INVALID'), names,
    'WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_INVENTORY_INVALID'
  );
  const staged = assertInventory(
    inventoryFromDirectory(payloadRoot, names), names,
    'WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_STAGED_INVENTORY_INVALID'
  ...
  const config = readJson(configPath, 'WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_CONFIG_INVALID');
  if (JSON.stringify(saved) !== JSON.stringify(staged) ||
      ...
      state.target_fingerprint !== binding.state.target_fingerprint) {
    fail('WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_STAGED_PAYLOAD_MISMATCH');
  }
...
function isolatedNativeInventory() {
  ...
  const temp = fs.mkdtempSync(path.join(os.tmpdir(), 'work-0037-codex-04-native-'));
  try {
    ...
    if (native.file_count !== 23 || native.names.filter((name) => name.endsWith('.gs')).length !== 22 ||
      ...
      fail('WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_NATIVE_INVENTORY_INVALID');
    }
    return {
      file_count: 23,
      gs_file_count: 22,
...
function pushPayload() {
  if (process.env[pushAllowedEnv] !== 'true' ||
    ...
    fail('WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_PUSH_OPT_IN_REQUIRED');
  }
  const staged = assertStaged();
  const automationOff = assertAutomationOffEvidence();
  const native = assertClaspNativePayloadSelection(workspaceRoot, staged.names);
  if (native.file_count !== 23) fail('WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_NATIVE_INVENTORY_INVALID');
  if (staged.state.phase !== 'STAGED' || staged.state.push_attempt_count !== 0 ||
      staged.state.pull_attempt_count !== 0) {
    fail('WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_PUSH_ALREADY_ATTEMPTED');
  }
  ...
  if (semantic.file_count !== 23 || semantic.gs_file_count !== 22 ||
    ...
    fail('WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_PUSH_INVENTORY_INVALID');
  }
...
function pullVerify() {
  if (process.env[pullAllowedEnv] !== 'true' ||
      process.env.GAS_DEV_CLASP_ALLOWED !== 'true') {
    fail('WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_PULL_OPT_IN_REQUIRED');
  }
  const staged = assertStaged();
  if (staged.state.phase !== 'PUSH_PASS' || staged.state.push_attempt_count !== 1 ||
      staged.state.pull_attempt_count !== 0) {
    fail('WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_PULL_ALREADY_ATTEMPTED');
  }
  const started = Object.assign({}, staged.state, {
  ...
  writeJsonAtomic(executionStatePath, started);
  if (fs.existsSync(pullRoot)) fail('WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_PULL_WORKSPACE_EXISTS');
  ...
  if (result.exit_code !== 0) fail('WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_CLASP_PULL_FAILED');
  const pulledRoot = path.join(pullRoot, 'payload');
  assertExactPayloadDirectory(
    pulledRoot, 'WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_PULLBACK_INVENTORY_INVALID', staged.names
  );
  const pulled = assertInventory(
    inventoryFromDirectory(pulledRoot, staged.names), staged.names,
    'WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_PULLBACK_INVENTORY_INVALID'
  );
  if (JSON.stringify(pulled) !== JSON.stringify(staged.inventory)) {
    fail('WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_PULLBACK_PARITY_FAILED');
  }
  Object.assign(started, {
...
function evidence() {
  const state = fs.existsSync(executionStatePath) ?
    readJson(executionStatePath, 'WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_EXECUTION_STATE_INVALID') : null;
...
async function main() {
  ...
  try {
    ...
    else fail('UNKNOWN_WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_COMMAND');
  } catch (error) {
    const code = error && error.code || 'WORK_0037_CODEX_04_IDLE_LOG_FINALIZATION_OPERATION_FAILED';
    safeWrite({
      lane: laneName,
      command,
      status: code,
