...
$oldPayloadRoot = 'implementation/GoogleSpreadsheet/release/v2.8.8-prepilot/apps-script'
$newPayloadRoot = 'implementation/GoogleSpreadsheet/release/v2.8.9-prepilot/apps-script'
$moduleRoot = [System.IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..'))
$repoRoot = [System.IO.Path]::GetFullPath((Join-Path $moduleRoot '../..'))
if (-not $TransferDirectory) { $TransferDirectory = Join-Path $moduleRoot 'transfer/v2.8.9-prepilot' }
...
if ($manifest.old_version -ne 'v2.8.8-prepilot' -or $manifest.new_version -ne 'v2.8.9-prepilot') { throw 'Patch manifest version binding mismatch.' }
...
if ($config.code_version -ne '2.8.9-prepilot' -or $config.schema_version -ne '2.6' -or $config.ai_schema_version -ne '2.0' -or $config.migration_version -ne '3' -or [bool]$config.test_mode -ne $true -or [bool]$config.automation_enabled -ne $false) { throw 'Expected post-update configuration mismatch.' }
...
foreach ($literal in @($companyReplacementHeading, $unchangedHeading, 'CODE_VERSION=2.8.9-prepilot', 'fixed T8', 'Release B9', 'TEST_MODE=true', 'AUTOMATION_ENABLED=false', 'S00-S80', 'S90/S99', 'old SHA-256', 'new SHA-256', $manualRepairText)) {
