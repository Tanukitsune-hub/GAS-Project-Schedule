...
$releaseVersion = 'v2.8.25-prepilot-phase8c'
$expectedCodeVersion = '2.8.25-prepilot'
...
$expectedPilotScope = 'AUTOMATIC_PERSONAL_INBOX_SHADOW_PILOT'
$expectedPilotAdmissionMode = 'AUTOMATIC_INBOX'
$expectedPilotSourceMode = 'AUTOMATIC_INBOX_PILOT'
...
$expectedPilotQuery = "in:inbox -in:spam -in:trash -category:promotions -category:social -label:$manualExcludeLabel"
...
function Convert-AuditedPhase8CConfig {
  param(
    [Parameter(Mandatory = $true)][string]$SourceConfig
  )
  $transformed = $SourceConfig
  $auditedTransforms = @(
    @{ Name = 'TEST_MODE'; Pattern = 'TEST_MODE:\s*true'; Replacement = 'TEST_MODE: false' },
    @{ Name = 'EXTERNAL_AI_ENABLED'; Pattern = 'EXTERNAL_AI_ENABLED:\s*false'; Replacement = 'EXTERNAL_AI_ENABLED: true' },
    @{ Name = 'EXTERNAL_AI_OPERATOR_APPROVED'; Pattern = 'EXTERNAL_AI_OPERATOR_APPROVED:\s*false'; Replacement = 'EXTERNAL_AI_OPERATOR_APPROVED: true' },
    @{ Name = 'EXTERNAL_AI_DATA_POLICY_APPROVED'; Pattern = 'EXTERNAL_AI_DATA_POLICY_APPROVED:\s*false'; Replacement = 'EXTERNAL_AI_DATA_POLICY_APPROVED: true' },
    @{ Name = 'EXTERNAL_AI_CREDENTIAL_STORAGE_APPROVED'; Pattern = 'EXTERNAL_AI_CREDENTIAL_STORAGE_APPROVED:\s*false'; Replacement = 'EXTERNAL_AI_CREDENTIAL_STORAGE_APPROVED: true' },
    @{ Name = 'EXTERNAL_AI_AUTH_CONFIGURED'; Pattern = 'EXTERNAL_AI_AUTH_CONFIGURED:\s*false'; Replacement = 'EXTERNAL_AI_AUTH_CONFIGURED: true' }
  )
  foreach ($transform in $auditedTransforms) {
    $matches = [regex]::Matches($transformed, $transform.Pattern)
    if ($matches.Count -ne 1) {
      throw "Expected exactly one audited Phase 8C source declaration: $($transform.Name)"
    }
  ...
  if ($transformed -notmatch 'AUTOMATION_ENABLED:\s*false') {
    throw 'Phase 8C Automation default must remain OFF.'
  }
  if ($transformed -notmatch 'EXTERNAL_AI_COMPANY_APPROVED:\s*false') {
    throw 'Legacy company approval compatibility alias must remain false.'
  }
  return $transformed
}
...
foreach ($literal in @(
    "AUTOMATION_PILOT_SCOPE: '$expectedPilotScope'",
    "AUTOMATION_PILOT_ADMISSION_MODE: '$expectedPilotAdmissionMode'",
    "AUTOMATION_PILOT_SOURCE_MODE: '$expectedPilotSourceMode'",
    'AUTOMATION_PILOT_GMAIL_QUERY:',
    $expectedPilotQuery,
    "AUTOMATION_PILOT_MAX_MESSAGES_PER_RUN: $expectedPilotMaxMessages"
  )) {
  if (-not $sourceConfig.Contains($literal)) {
    throw "Personal Shadow Pilot configuration invariant failed: $literal"
  }
}
...
$phase8cConfig = Convert-AuditedPhase8CConfig -SourceConfig $sourceConfig
if (
  $phase8cConfig -notmatch 'TEST_MODE:\s*false' -or
  $phase8cConfig -match 'TEST_MODE:\s*true' -or
  $phase8cConfig -notmatch 'EXTERNAL_AI_ENABLED:\s*true' -or
  $phase8cConfig -notmatch 'EXTERNAL_AI_OPERATOR_APPROVED:\s*true' -or
  $phase8cConfig -notmatch 'EXTERNAL_AI_DATA_POLICY_APPROVED:\s*true' -or
  $phase8cConfig -notmatch 'EXTERNAL_AI_CREDENTIAL_STORAGE_APPROVED:\s*true' -or
  $phase8cConfig -notmatch 'EXTERNAL_AI_AUTH_CONFIGURED:\s*true'
) {
  throw 'Phase 8C audited production configuration transformation failed.'
}
...
if ($payloadConfig -ne $phase8cConfig) {
  throw 'Phase 8C Config differs from the audited production configuration transform.'
}
...
configuration flags required for the automatic personal Inbox Shadow Pilot boundary.
...
[pscustomobject]@{
  ...
  HighestLocalStatus = 'READY_FOR_USER_AUTOMATIC_INBOX_SHADOW_PILOT'
} | Format-List
