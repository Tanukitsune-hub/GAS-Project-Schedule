...
$oldVersion = 'v2.8.7-prepilot'
$newVersion = 'v2.8.8-prepilot'
...
if (-not $OutputDirectory) { $OutputDirectory = Join-Path $moduleRoot 'transfer/v2.8.8-prepilot' }
...
$expectedConfig = [ordered]@{code_version='2.8.8-prepilot';schema_version='2.6';ai_schema_version='2.0';migration_version='3';test_mode=$true;automation_enabled=$false}
$manifest = [ordered]@{
...
$mdLines = @(
  ...
  '## Post-update configuration checks','','- CODE_VERSION=2.8.8-prepilot','- SCHEMA_VERSION=2.6','- AI_SCHEMA_VERSION=2.0','- MIGRATION_VERSION=3','- TEST_MODE=true','- AUTOMATION_ENABLED=false','',
