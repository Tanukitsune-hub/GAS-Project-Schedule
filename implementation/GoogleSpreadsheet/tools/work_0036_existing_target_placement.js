...
/**
 * Work 0036-only Phase 8C placement lane.
 *
 * This lane reuses only the completed Work 0010 personal-synthetic binding.
 * Work 0033 is checked as historical placement evidence, never as mutation
 * authority. The payload is the audited Phase 8C release package: exactly 22
...
const phase8cReleaseRelativeRoot =
  'implementation/GoogleSpreadsheet/release/v2.8.21-prepilot-phase8c/apps-script';
const phase8cReleaseRoot = path.join(
  repositoryRoot, phase8cReleaseRelativeRoot.replaceAll('/', path.sep)
);
const workspaceName = '.clasp-work-0036';
const pullWorkspaceName = '.clasp-pull-verify-work-0036';
...
const previousPlacementStatePath = path.join(
  moduleRoot, '.clasp-work-0033', 'work-0033-execution-state.json'
);
const exactBranch = 'codex/0036-personal-automation-qualification';
const instructionHead = 'ea484cf3e7cef3b5e67d15eebd7b2aac03c1ec6a';
const phase8cSchema = 'WORK_OS_PERSONAL_AUTOMATION_QUALIFICATION_PLACEMENT_V1';
...
function assertExactBranchCleanAndPublished(expectedHead) {
  if (String(git(['branch', '--show-current'])).trim() !== exactBranch) {
    fail('WORK_0036_EXACT_BRANCH_REQUIRED');
  }
  if (String(git([
  ...
  if (ancestry.error || ancestry.status !== 0) {
    fail('WORK_0036_INSTRUCTION_ANCESTRY_INVALID');
  }
  const remote = String(git(['rev-parse', `origin/${exactBranch}`])).trim();
  if (remote !== head) fail('WORK_0036_REPAIR_HEAD_NOT_PUBLISHED');
  if (expectedHead && expectedHead !== head) {
    fail('WORK_0036_REPAIR_HEAD_CHANGED');
  }
  return head;
}
...
function acquireOperationLock(command) {
  ...
  try {
    ...
    fail('WORK_0036_OPERATION_ALREADY_RUNNING');
  }
  return () => {
    try {
...
function phase8cPayloadNames() {
  ...
  if (!valid) fail('WORK_0036_PHASE8C_PAYLOAD_INVENTORY_INVALID');
  return names;
}
...
function committedPhase8cBuffer(name) {
  ...
  if (result.error || result.status !== 0) {
    fail('WORK_0036_COMMITTED_PHASE8C_READ_FAILED');
  }
  return result.stdout;
}
...
function assertIdentifierNotTracked(identifier) {
  ...
  if (result.status === 0) fail('WORK_0036_TARGET_IDENTIFIER_IS_TRACKED');
  if (result.status !== 1) fail('WORK_0036_TRACKED_IDENTIFIER_SCAN_FAILED');
}
...
function assertExistingBindingObjects(config, target, state) {
  ...
  if (!valid) fail('WORK_0036_EXISTING_TARGET_BINDING_INVALID');
...
function assertPreviousPlacement(binding, previousPlacement) {
  const valid = previousPlacement &&
    previousPlacement.schema ===
      'WORK_OS_GEMINI_SCHEMA_COMPATIBILITY_PLACEMENT_V1' &&
    previousPlacement.work_id === '0033' &&
    previousPlacement.source_binding_work_id === '0010' &&
    previousPlacement.previous_placement_work_id === '0032' &&
  ...
  if (!valid) fail('WORK_0036_PREVIOUS_PLACEMENT_STATE_INVALID');
  return previousPlacement;
}

function loadExistingBinding() {
  const binding = assertExistingBindingObjects(
    readJson(sourceConfigPath, 'WORK_0036_EXISTING_TARGET_CONFIG_MISSING'),
    readJson(sourceTargetPath, 'WORK_0036_EXISTING_TARGET_METADATA_MISSING'),
    readJson(sourceStatePath, 'WORK_0036_EXISTING_TARGET_STATE_MISSING')
  );
  ...
  binding.previousPlacement = assertPreviousPlacement(binding, readJson(
    ...
    'WORK_0036_PREVIOUS_PLACEMENT_STATE_MISSING'
  ));
  return binding;
}
...
function initialExecutionState(repairHead, inventory, binding) {
  return {
    ...
    previous_placement_work_id: '0033',
...
function assertStateBase(state) {
  const valid = state && state.schema === phase8cSchema &&
    ...
    state.previous_placement_work_id === '0033' &&
  ...
  if (!valid) fail('WORK_0036_EXECUTION_STATE_INVALID');
}
...
function nextAttemptState(state, command) {
  ...
  if (command === 'push') {
    if (state.phase !== 'STAGED' || state.push_attempt_count !== 0 ||
      ...
      fail('WORK_0036_PUSH_ALREADY_ATTEMPTED');
    }
  ...
  if (command === 'pull-verify') {
    if (state.phase !== 'PUSH_PASS' || state.push_attempt_count !== 1 ||
        state.pull_attempt_count !== 0) {
      fail('WORK_0036_PULL_ALREADY_ATTEMPTED');
    }
    next.pull_attempt_count = 1;
    next.phase = 'PULL_ATTEMPT_STARTED';
    return next;
  }
  fail('WORK_0036_REMOTE_COMMAND_INVALID');
}
...
function stagePayload() {
  ...
  if (fs.existsSync(workspaceRoot) || fs.existsSync(pullRoot)) {
    fail('WORK_0036_LOCAL_STATE_ALREADY_EXISTS');
  }
  ...
  if (JSON.stringify(staged) !== JSON.stringify(committed)) {
    fail('WORK_0036_STAGED_PAYLOAD_SOURCE_SKEW');
  }
...
function assertStagedPayload() {
  if (!fs.existsSync(payloadRoot) || !fs.existsSync(inventoryPath) ||
    ...
    fail('WORK_0036_STAGED_PAYLOAD_MISSING');
  }
  ...
  if (config.scriptId !== binding.config.scriptId ||
    ...
    fail('WORK_0036_EXISTING_TARGET_CHANGED');
  }
  try {
...
function isolatedNativeSelection() {
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'work-0036-native-'));
  try {
...
function pushPayload() {
  if (process.env.GAS_WORK_0036_PUSH_ALLOWED !== 'true' ||
  ...
  if (semantic.file_count !== 23 || semantic.gs_file_count !== 22 ||
    ...
    fail('WORK_0036_PUSH_SEMANTIC_INVENTORY_INVALID');
  }
...
function preparePullWorkspace(config, names) {
  if (fs.existsSync(pullRoot)) fail('WORK_0036_PULL_WORKSPACE_ALREADY_EXISTS');
...
function pullVerify() {
  if (process.env.GAS_WORK_0036_PULL_ALLOWED !== 'true' ||
      process.env.GAS_DEV_CLASP_ALLOWED !== 'true') {
  ...
  if (result.exit_code !== 0) fail('WORK_0036_CLASP_PULL_FAILED');
  const pulledRoot = path.join(pullRoot, 'payload');
  assertExactPayloadDirectory(
    pulledRoot, 'WORK_0036_PULLBACK_UNEXPECTED_CONTENT', staged.names
  );
  ...
  if (JSON.stringify(pulled) !== JSON.stringify(staged.inventory)) {
    fail('WORK_0036_PULLBACK_PARITY_FAILED');
  }
  Object.assign(started, {
...
function safeAttemptEvidence(statePath = executionStatePath) {
  ...
  try {
    const state = readJson(statePath, 'WORK_0036_EXECUTION_STATE_INVALID');
    assertStateBase(state);
    return {
...
async function main() {
  ...
  try {
    ...
    else fail('UNKNOWN_WORK_0036_COMMAND');
  } catch (error) {
