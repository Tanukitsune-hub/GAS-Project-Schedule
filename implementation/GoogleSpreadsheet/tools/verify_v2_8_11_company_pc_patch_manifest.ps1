...
if (-not $TransferDirectory) { $TransferDirectory = Join-Path $moduleRoot 'transfer/v2.8.11-prepilot' }
...
if ($manifest.old_version -ne 'v2.8.10-prepilot' -or $manifest.new_version -ne 'v2.8.11-prepilot') { throw 'Patch manifest version binding mismatch.' }
...
if ($config.code_version -ne '2.8.11-prepilot' -or $config.schema_version -ne '2.6' -or $config.ai_schema_version -ne '2.0' -or $config.migration_version -ne '3' -or [bool]$config.test_mode -ne $true -or [bool]$config.automation_enabled -ne $false) { throw 'Expected post-update configuration mismatch.' }
...
foreach ($literal in @($companyReplacementHeading, $unchangedHeading, 'CODE_VERSION=2.8.11-prepilot', 'fixed T10', 'Release B11', 'TEST_MODE=true', 'AUTOMATION_ENABLED=false', 'already complete through S99', 'Do not run Setup', 'old SHA-256', 'new SHA-256', $manualRepairText)) {
