...
/**
 * Work 0036 review-fix replacement-lane contract tests. This suite is local
 * only and uses placeholder binding metadata; it never invokes Google APIs.
 */
...
const placement = require('../tools/work_0036_review_fix_placement');
const {
  claspProjectConfig,
...
const previousPlacement = {
  schema: 'WORK_OS_PERSONAL_AUTOMATION_QUALIFICATION_PLACEMENT_V1',
  work_id: '0036',
...
assert.strictEqual(placement.workspaceName, '.clasp-work-0036-review-fix');
assert.strictEqual(
  placement.pullWorkspaceName, '.clasp-pull-verify-work-0036-review-fix'
);
assert.strictEqual(
...
assert.strictEqual(
  staged.schema, 'WORK_OS_PERSONAL_AUTOMATION_REVIEW_FIX_REPLACEMENT_V1'
);
assert.strictEqual(staged.previous_placement_work_id, '0036');
assert.strictEqual(staged.replacement_tranche, 'REVIEW_FIX_REPLACEMENT');
...
const toolSource = fs.readFileSync(
  path.join(__dirname, '..', 'tools', 'work_0036_review_fix_placement.js'),
  'utf8'
);
assert.ok(toolSource.includes('GAS_WORK_0036_REVIEW_FIX_PUSH_ALLOWED'));
...
const nativeRoot = fs.mkdtempSync(path.join(
  os.tmpdir(), 'work-0036-review-fix-native-test-'
));
try {
...
const evidenceRoot = fs.mkdtempSync(path.join(
  os.tmpdir(), 'work-0036-review-fix-evidence-'
));
try {
...
process.stdout.write(`${JSON.stringify({
  suite: 'work_0036_review_fix_placement',
