...
const {
  ...
  safeAttemptEvidence,
  normalizeCommand
} = require('../tools/work_0029_existing_target_placement');
...
test('WORK_0029_HAS_DISTINCT_ONE_USE_LOCAL_STATE', () => {
  assert.strictEqual(workspaceName, '.clasp-work-0029');
  assert.strictEqual(pullWorkspaceName, '.clasp-pull-verify-work-0029');
  assert.strictEqual(executionStateFileName, 'work-0029-execution-state.json');
...
test('EXACT_BRANCH_AND_INSTRUCTION_HEAD_ARE_PINNED', () => {
  assert.strictEqual(exactBranch, 'codex/0029-gemini-runtime-activation-remediation');
  assert.strictEqual(instructionHead, 'cbfa13f5f80a6e3902f2e3387488fed61202fb67');
...
test('ONLY_COMPLETED_WORK_0010_BINDING_IS_ACCEPTED', () => {
  ...
  assert.throws(
    ...
    (error) => error instanceof GateError &&
      error.code === 'WORK_0029_EXISTING_TARGET_BINDING_INVALID'
  );
  assert.throws(
...
test('WORK_0029_STATE_CONTAINS_NO_TARGET_OR_ACCOUNT_IDENTIFIER', () => {
  assert.strictEqual(stagedState().previous_placement_work_id, '0028');
...
test('PUSH_MARKER_IS_ATOMIC_AND_ONE_USE', () => {
  ...
  assert.throws(
    ...
    (error) => error instanceof GateError &&
      error.code === 'WORK_0029_PUSH_ALREADY_ATTEMPTED'
  );
});
...
test('OPTIONAL_PULL_REQUIRES_PUSH_PASS_AND_IS_ONE_USE', () => {
  assert.throws(
    ...
    (error) => error instanceof GateError &&
      error.code === 'WORK_0029_PULL_ALREADY_ATTEMPTED'
  );
...
test('OPERATION_LOCK_REFUSES_CONCURRENCY', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'work-0029-lock-'));
  ...
  try {
    ...
    assert.throws(
      ...
      (error) => error instanceof GateError &&
        error.code === 'WORK_0029_OPERATION_ALREADY_RUNNING'
    );
    release();
  } finally {
...
test('SAFE_EVIDENCE_EXCLUDES_BINDING_IDENTIFIERS', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'work-0029-evidence-'));
...
process.stdout.write(`${JSON.stringify({
  suite: 'work_0029_existing_target_placement',
