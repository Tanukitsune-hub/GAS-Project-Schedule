...
test('R4-06A_CURRENT_CONFIG_USES_2_8_25_SCHEMA_2_6_MIGRATION_3', () => {
  const source = read('apps-script-v2/00_Config.gs');
  assert.match(source, /CODE_VERSION:\s*'2\.8\.25-prepilot'/);
...
test('R4-06C_CURRENT_README_AND_CHANGELOG_PRESERVE_R4_01_TO_R4_06', () => {
  ...
  assert.ok(readme.includes('2.8.25-prepilot'));
...
test('R4-06D_MANUAL_GUIDE_USES_CURRENT_VERSION_AND_NO_GO_STATUS_CAP', () => {
  ...
  assert.ok(guide.includes('2.8.25-prepilot'));
