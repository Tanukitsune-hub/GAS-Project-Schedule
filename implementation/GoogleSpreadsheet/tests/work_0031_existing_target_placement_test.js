...
const {
  ...
  safeAttemptEvidence,
  normalizeCommand
} = require('../tools/work_0031_existing_target_placement');
...
test('WORK_0031_HAS_DISTINCT_ONE_USE_LOCAL_STATE', () => {
  assert.strictEqual(workspaceName, '.clasp-work-0031');
  assert.strictEqual(pullWorkspaceName, '.clasp-pull-verify-work-0031');
  assert.strictEqual(executionStateFileName, 'work-0031-execution-state.json');
...
test('EXACT_BRANCH_AND_INSTRUCTION_HEAD_ARE_PINNED', () => {
  assert.strictEqual(exactBranch, 'codex/0031-gemini-v1beta-endpoint-runtime-validation');
  assert.strictEqual(instructionHead, '94da55359c07c8e48b66b8ce68204bbaaf15547d');
...
test('ONLY_COMPLETED_WORK_0010_BINDING_IS_ACCEPTED', () => {
  ...
  assert.throws(
    ...
    (error) => error instanceof GateError &&
      error.code === 'WORK_0031_EXISTING_TARGET_BINDING_INVALID'
  );
  assert.throws(
...
test('ONLY_COMPLETED_WORK_0030_PLACEMENT_CAN_AUTHORIZE_THIS_LANE', () => {
  assert.deepStrictEqual(
  ...
  assert.throws(
    ...
    (error) => error instanceof GateError &&
      error.code === 'WORK_0031_PREVIOUS_PLACEMENT_STATE_INVALID'
  );
  assert.throws(
    () => assertPreviousPlacement({ state: sourceState }, Object.assign(
...
test('WORK_0031_STATE_CONTAINS_NO_TARGET_OR_ACCOUNT_IDENTIFIER', () => {
  assert.strictEqual(stagedState().previous_placement_work_id, '0030');
...
test('PUSH_MARKER_IS_ATOMIC_AND_ONE_USE', () => {
  ...
  assert.throws(
    ...
    (error) => error instanceof GateError &&
      error.code === 'WORK_0031_PUSH_ALREADY_ATTEMPTED'
  );
});
...
test('OPTIONAL_PULL_REQUIRES_PUSH_PASS_AND_IS_ONE_USE', () => {
  assert.throws(
    ...
    (error) => error instanceof GateError &&
      error.code === 'WORK_0031_PULL_ALREADY_ATTEMPTED'
  );
...
test('PULL_ATTEMPT_IS_RECORDED_BEFORE_PERSISTENT_PULL_WORKSPACE_CREATION', () => {
  const source = fs.readFileSync(path.join(
    __dirname, '..', 'tools', 'work_0031_existing_target_placement.js'
  ), 'utf8');
...
test('OPERATION_LOCK_REFUSES_CONCURRENCY', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'work-0031-lock-'));
  ...
  try {
    ...
    assert.throws(
      ...
      (error) => error instanceof GateError &&
        error.code === 'WORK_0031_OPERATION_ALREADY_RUNNING'
    );
    release();
  } finally {
...
test('SAFE_EVIDENCE_EXCLUDES_BINDING_IDENTIFIERS', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'work-0031-evidence-'));
...
process.stdout.write(`${JSON.stringify({
  suite: 'work_0031_existing_target_placement',
