...
const expected = {
  code: '2.8.16-prepilot', schema: '2.6', aiSchema: '2.0', migration: '3',
...
check(() => assert.match(config, /CODE_VERSION:\s*'2\.8\.16-prepilot'/));
...
const activeDocuments = [
  ...
  'implementation/GoogleSpreadsheet/tools/v2_8_16/DEPLOYMENT_MANIFEST.template.md',
  'implementation/GoogleSpreadsheet/tools/v2_8_16/MANUAL_ACCEPTANCE_GUIDE.md',
  'implementation/GoogleSpreadsheet/tools/v2_8_16/SANDBOX_QUICKSTART.md'
];
...
const visualizationPaths = [
  ...
  'implementation/GoogleSpreadsheet/visualizations/task_authority_protocol_v2_8_16.html'
];
for (const file of visualizationPaths) {
  const content = read(file);
  check(() => assert.match(content, /data-code-version="2\.8\.16-prepilot"/));
...
check(() => assert.strictEqual(
  fs.existsSync(path.join(moduleRoot, 'transfer', 'v2.8.16-prepilot')),
  false,
...
const releasePaths = [
  path.join(moduleRoot, 'release', 'v2.8.16-prepilot'),
  path.join(moduleRoot, 'release', 'v2.8.16-prepilot-phase8c')
];
...
if (!currentReleasePresent) {
  check(() => assert.ok(releasePaths.every((target) => !fs.existsSync(target)),
    'Source A16 must not contain generated v2.8.16 release packages'));
} else {
  ...
  check(() => assert.ok(releasePaths.every((target) =>
    ...
    'Release B16 must contain both generated packages'));
}

process.stdout.write(`${JSON.stringify({
  suite: 'canonical_document_consistency', passed, failed: 0,
  release_stage: currentReleasePresent ? 'B16_OR_LATER' : 'A16_SOURCE'
}, null, 2)}\n`);
