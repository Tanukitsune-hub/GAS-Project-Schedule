...
const expected = {
  code: '2.8.19-prepilot', schema: '2.6', aiSchema: '2.0', migration: '3',
...
check(() => assert.match(config, /CODE_VERSION:\s*'2\.8\.19-prepilot'/));
...
for (const file of visualizationPaths) {
  const content = read(file);
  check(() => assert.match(content, /data-code-version="2\.8\.19-prepilot"/));
...
check(() => assert.strictEqual(
  fs.existsSync(path.join(moduleRoot, 'transfer', 'v2.8.19-prepilot')),
  false,
...
const releasePaths = [
  path.join(moduleRoot, 'release', 'v2.8.19-prepilot'),
  path.join(moduleRoot, 'release', 'v2.8.19-prepilot-phase8c')
];
...
if (!currentReleasePresent) {
  check(() => assert.ok(releasePaths.every((target) => !fs.existsSync(target)),
    'Source A19 must not contain generated v2.8.19 release packages'));
} else {
  ...
  check(() => assert.ok(releasePaths.every((target) =>
    ...
    'Release B19 must contain both generated packages'));
}

process.stdout.write(`${JSON.stringify({
  suite: 'canonical_document_consistency', passed, failed: 0,
  release_stage: currentReleasePresent ? 'B19_OR_LATER' : 'A19_SOURCE'
}, null, 2)}\n`);
