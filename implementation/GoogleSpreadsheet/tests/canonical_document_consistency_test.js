...
const expected = {
  code: '2.8.14-prepilot', schema: '2.6', aiSchema: '2.0', migration: '3',
...
check(() => assert.match(config, /CODE_VERSION:\s*'2\.8\.14-prepilot'/));
...
const activeDocuments = [
  ...
  'implementation/GoogleSpreadsheet/tools/v2_8_14/DEPLOYMENT_MANIFEST.template.md',
  'implementation/GoogleSpreadsheet/tools/v2_8_14/MANUAL_ACCEPTANCE_GUIDE.md',
  'implementation/GoogleSpreadsheet/tools/v2_8_14/SANDBOX_QUICKSTART.md'
];
...
const visualizationPaths = [
  ...
  'implementation/GoogleSpreadsheet/visualizations/task_authority_protocol_v2_8_14.html'
];
for (const file of visualizationPaths) {
  const content = read(file);
  check(() => assert.match(content, /data-code-version="2\.8\.14-prepilot"/));
...
check(() => assert.strictEqual(
  fs.existsSync(path.join(moduleRoot, 'transfer', 'v2.8.14-prepilot')),
  false,
...
const releasePaths = [
  path.join(moduleRoot, 'release', 'v2.8.14-prepilot'),
  path.join(moduleRoot, 'release', 'v2.8.14-prepilot-phase8c')
];
...
if (!currentReleasePresent) {
  check(() => assert.ok(releasePaths.every((target) => !fs.existsSync(target)),
    'Source A14 must not contain generated v2.8.14 release packages'));
} else {
  ...
  check(() => assert.ok(releasePaths.every((target) =>
    ...
    'Release B14 must contain both generated packages'));
}

process.stdout.write(`${JSON.stringify({
  suite: 'canonical_document_consistency', passed, failed: 0,
  release_stage: currentReleasePresent ? 'B14_OR_LATER' : 'A14_SOURCE'
}, null, 2)}\n`);
