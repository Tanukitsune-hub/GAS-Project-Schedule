...
const expected = {
  code: '2.8.27-prepilot', schema: '2.6', aiSchema: '2.0', migration: '3',
...
check(() => assert.match(config, /CODE_VERSION:\s*'2\.8\.27-prepilot'/));
...
const activeDocuments = [
  ...
  'implementation/GoogleSpreadsheet/tools/v2_8_27/DEPLOYMENT_MANIFEST.template.md',
  'implementation/GoogleSpreadsheet/tools/v2_8_27/MANUAL_ACCEPTANCE_GUIDE.md',
  'implementation/GoogleSpreadsheet/tools/v2_8_27/SANDBOX_QUICKSTART.md'
];
...
for (const file of visualizationPaths) {
  const content = read(file);
  check(() => assert.match(content, /data-code-version="2\.8\.27-prepilot"/));
...
const releasePaths = [
  path.join(moduleRoot, 'release', 'v2.8.27-prepilot'),
  path.join(moduleRoot, 'release', 'v2.8.27-prepilot-phase8c')
];
...
if (!currentReleasePresent) {
  check(() => assert.ok(releasePaths.every((target) => !fs.existsSync(target)),
    'Work 0041 source must not contain generated v2.8.27 release packages'));
} else {
  ...
  check(() => assert.ok(releasePaths.every((target) =>
    ...
    'Work 0041 must contain both generated packages'));
}
