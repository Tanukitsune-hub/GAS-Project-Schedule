...
const expected = {
  code: '2.8.23-prepilot', schema: '2.6', aiSchema: '2.0', migration: '3',
...
check(() => assert.match(config, /CODE_VERSION:\s*'2\.8\.23-prepilot'/));
...
for (const file of visualizationPaths) {
  const content = read(file);
  check(() => assert.match(content, /data-code-version="2\.8\.23-prepilot"/));
...
check(() => assert.strictEqual(
  fs.existsSync(path.join(moduleRoot, 'transfer', 'v2.8.23-prepilot')),
  false,
...
const releasePaths = [
  path.join(moduleRoot, 'release', 'v2.8.23-prepilot'),
  path.join(moduleRoot, 'release', 'v2.8.23-prepilot-phase8c')
];
...
if (!currentReleasePresent) {
  check(() => assert.ok(releasePaths.every((target) => !fs.existsSync(target)),
    'Work 0037 source must not contain generated v2.8.23 release packages'));
} else {
  ...
  check(() => assert.ok(releasePaths.every((target) =>
    ...
    'Release B22 must contain both generated packages'));
}
