...
const placement = require('../tools/work_0037_automatic_inbox_shadow_pilot_placement');

const toolSource = fs.readFileSync(
  path.join(__dirname, '..', 'tools',
    'work_0037_automatic_inbox_shadow_pilot_placement.js'),
  'utf8'
);
...
assert.match(toolSource, /WORK_OS_AUTOMATIC_INBOX_SHADOW_PILOT_REPLACEMENT_V1/);
assert.match(toolSource, /AUTOMATIC_INBOX_AUTOMATION_OFF_CONFIRMED/);
...
assert.throws(
  ...
  (error) => error && error.code ===
    'WORK_0037_AUTOMATIC_INBOX_EXECUTION_STATE_INVALID'
);

process.stdout.write(`${JSON.stringify({
  suite: 'work_0037_automatic_inbox_shadow_pilot_placement',
