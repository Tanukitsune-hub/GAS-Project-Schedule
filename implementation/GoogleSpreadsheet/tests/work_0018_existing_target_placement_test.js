...
const {
  ...
  safeAttemptEvidence,
  normalizeCommand
} = require('../tools/work_0018_existing_target_placement');
...
test('WORK_0018_HAS_DISTINCT_ONE_USE_LOCAL_STATE', () => {
  assert.strictEqual(workspaceName, '.clasp-work-0018');
  assert.strictEqual(pullWorkspaceName, '.clasp-pull-verify-work-0018');
  assert.strictEqual(executionStateFileName, 'work-0018-execution-state.json');
  assert.notStrictEqual(workspaceName, '.clasp-work-0010');
  assert.notStrictEqual(workspaceName, '.clasp-work-0016');
});

test('EXACT_BRANCH_AND_INSTRUCTION_HEAD_ARE_PINNED', () => {
  assert.strictEqual(exactBranch, 'codex/0018-advanced-gmail-byte-body-repair');
  assert.strictEqual(instructionHead, '3b91f44abe51c68b73b32de62215d998e73a14ca');
...
test('ONLY_COMPLETED_WORK_0010_BINDING_IS_ACCEPTED', () => {
  ...
  assert.throws(
    ...
    (error) => error instanceof GateError &&
      error.code === 'WORK_0018_EXISTING_TARGET_BINDING_INVALID'
  );
  assert.throws(
...
test('WORK_0018_STATE_CONTAINS_NO_TARGET_OR_ACCOUNT_IDENTIFIER', () => {
  assert.strictEqual(stagedState().previous_placement_work_id, '0016');
...
test('PUSH_MARKER_IS_ATOMIC_AND_ONE_USE', () => {
  ...
  assert.throws(
    ...
    (error) => error instanceof GateError &&
      error.code === 'WORK_0018_PUSH_ALREADY_ATTEMPTED'
  );
});
...
test('OPTIONAL_PULL_REQUIRES_PUSH_PASS_AND_IS_ONE_USE', () => {
  assert.throws(
    ...
    (error) => error instanceof GateError &&
      error.code === 'WORK_0018_PULL_ALREADY_ATTEMPTED'
  );
...
test('OPERATION_LOCK_REFUSES_CONCURRENCY', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'work-0018-lock-'));
  ...
  try {
    ...
    assert.throws(
      ...
      (error) => error instanceof GateError &&
        error.code === 'WORK_0018_OPERATION_ALREADY_RUNNING'
    );
    release();
  } finally {
...
test('SAFE_EVIDENCE_EXCLUDES_BINDING_IDENTIFIERS', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'work-0018-evidence-'));
...
process.stdout.write(`${JSON.stringify({
  suite: 'work_0018_existing_target_placement',
