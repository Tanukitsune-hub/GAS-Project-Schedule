...
const {
  ...
  safeAttemptEvidence,
  normalizeCommand
} = require('../tools/work_0030_existing_target_placement');
...
test('WORK_0030_HAS_DISTINCT_ONE_USE_LOCAL_STATE', () => {
  assert.strictEqual(workspaceName, '.clasp-work-0030');
  assert.strictEqual(pullWorkspaceName, '.clasp-pull-verify-work-0030');
  assert.strictEqual(executionStateFileName, 'work-0030-execution-state.json');
  assert.notStrictEqual(workspaceName, '.clasp-work-0010');
  assert.notStrictEqual(workspaceName, '.clasp-work-0028');
  assert.notStrictEqual(workspaceName, '.clasp-work-0029');
});

test('EXACT_BRANCH_AND_INSTRUCTION_HEAD_ARE_PINNED', () => {
  assert.strictEqual(exactBranch, 'codex/0030-gemini-thinking-step-parser-remediation');
  assert.strictEqual(instructionHead, 'e692958d30f34c8e8834b81bcd1d8a9e00049ee6');
...
test('ONLY_COMPLETED_WORK_0010_BINDING_IS_ACCEPTED', () => {
  ...
  assert.throws(
    ...
    (error) => error instanceof GateError &&
      error.code === 'WORK_0030_EXISTING_TARGET_BINDING_INVALID'
  );
  assert.throws(
...
test('ONLY_COMPLETED_WORK_0029_PLACEMENT_CAN_AUTHORIZE_THIS_LANE', () => {
  assert.deepStrictEqual(
  ...
  assert.throws(
    ...
    (error) => error instanceof GateError &&
      error.code === 'WORK_0030_PREVIOUS_PLACEMENT_STATE_INVALID'
  );
  assert.throws(
    () => assertPreviousPlacement({ state: sourceState }, Object.assign(
...
test('WORK_0030_STATE_CONTAINS_NO_TARGET_OR_ACCOUNT_IDENTIFIER', () => {
  assert.strictEqual(stagedState().previous_placement_work_id, '0029');
...
test('PUSH_MARKER_IS_ATOMIC_AND_ONE_USE', () => {
  ...
  assert.throws(
    ...
    (error) => error instanceof GateError &&
      error.code === 'WORK_0030_PUSH_ALREADY_ATTEMPTED'
  );
});
...
test('OPTIONAL_PULL_REQUIRES_PUSH_PASS_AND_IS_ONE_USE', () => {
  assert.throws(
    ...
    (error) => error instanceof GateError &&
      error.code === 'WORK_0030_PULL_ALREADY_ATTEMPTED'
  );
...
test('PULL_ATTEMPT_IS_RECORDED_BEFORE_PERSISTENT_PULL_WORKSPACE_CREATION', () => {
  const source = fs.readFileSync(path.join(
    __dirname, '..', 'tools', 'work_0030_existing_target_placement.js'
  ), 'utf8');
...
test('OPERATION_LOCK_REFUSES_CONCURRENCY', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'work-0030-lock-'));
  ...
  try {
    ...
    assert.throws(
      ...
      (error) => error instanceof GateError &&
        error.code === 'WORK_0030_OPERATION_ALREADY_RUNNING'
    );
    release();
  } finally {
...
test('SAFE_EVIDENCE_EXCLUDES_BINDING_IDENTIFIERS', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'work-0030-evidence-'));
...
process.stdout.write(`${JSON.stringify({
  suite: 'work_0030_existing_target_placement',
