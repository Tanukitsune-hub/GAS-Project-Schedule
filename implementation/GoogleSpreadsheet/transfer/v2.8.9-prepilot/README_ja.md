# Phase 8B v2.8.9 Sandbox 再搬入資料

このフォルダはCode `2.8.9-prepilot`の非機密Phase 8B packageを、別途承認
された経路で会社PCへ搬入するためのoperator資料です。実行承認ではありません。
Automationは`OFF`のままです。

## 搬入前の確認

1. `TRANSFER_MANIFEST.md`のCorrected Source A9.1、Corrected Release B9.1、transfer self referenceを
   Gitの解決結果と照合する。
