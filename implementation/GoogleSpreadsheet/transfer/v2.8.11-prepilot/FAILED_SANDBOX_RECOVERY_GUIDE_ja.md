# T11 governing recovery boundary

The completed Sandbox is not a Setup-resume target. For this visibility-only
patch, STOP on an old/new hash mismatch, incomplete bounded ID list, malformed
summary, genuine Diagnostic FAIL, or any unexpected side-effect Boolean.

Do not run Setup, S90/S99, migration, Dashboard refresh, or manual repair. Do
not delete or recreate labels, Calendar, Properties, triggers, Task rows,
Ledger, Dashboard, protections, or Automation. Do not retry in the same
session. A rollback requires a separately approved procedure using verified
T10 bytes; this guide neither performs nor authorizes it.
