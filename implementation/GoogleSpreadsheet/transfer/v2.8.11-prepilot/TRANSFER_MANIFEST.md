# T11 governing transfer manifest
...
| Fixed transfer T11 | `SELF (the Git commit containing this transfer envelope)` |
| Candidate | `2.8.11-prepilot` / Schema `2.6` / AI Schema `2.0` / Migration `3` |
| Historical baseline | fixed T10 `927d8567bce64461840cc6f72fbae0c1e636a8e6` |
| Payload comparison | raw Git blob bytes / SHA-256 |
| Gate in this envelope | `PHASE8B_SANDBOX_NO_GO_T1_01_SUMMARY` |
| Automation | `OFF` |
| Real Workspace retransfer | `NOT_EXECUTED` |

The 11-file flat envelope is complete only when the generated patch manifests
and transfer checksum file are present and verified. This envelope carries a
controlled T1-01 re-observation only; it does not authorize Setup, repair, or
any subsequent Tranche action.
