# Work 0037 Manual Acceptance Guide - 2.8.24-prepilot
...
Required candidate: Code `2.8.24-prepilot`, Schema `2.6`, AI Schema `2.0`,
...
requires the actual Automation-OFF guard, a Thread-wide `手動/除外` veto,
spam/trash, non-Inbox, Promotions, Social, newsletter/list, and Google
Calendar notification hard exclusions; `手動/取込` is optional priority only.
A later separately authorized Work may permit one bounded personal runtime
action. Credential values must not enter source, evidence,
...
or identifiers, the exact automatic-Inbox candidate guard, the pilot-start
boundary contract, and no scheduled trigger.
