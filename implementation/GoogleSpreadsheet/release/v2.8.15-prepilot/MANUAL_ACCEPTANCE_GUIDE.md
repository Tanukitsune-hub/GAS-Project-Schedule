...
# 2.8.15-prepilot Manual Acceptance Guide
...
| Highest status allowed here | `READY_FOR_CONTROLLED_SANDBOX_VALIDATION` |
...
There is no active transfer or deployment target. Any future Sandbox activity
requires a separate authorization that names the target, functions, and attempt
limit. A version-property mismatch remains an explicit WARN and must not be
silently reconciled.
...
5. Confirm `apps-script/` contains exactly 23 `.gs` files and
...
3. Confirm `繧ｿ繧ｹ繧ｯ荳隕ｧ` has exactly 50 physical columns.
...
6. For a valid S00-S80 resume only, confirm Setup first proves the complete
   ...
   the exact 17ﾃ・ system block to the canonical plain-text format.
...
3. Confirm the Source A15 commit contains source/tests/tools/canonical docs/
   changelog/visualization/design/migration only.
4. Confirm the direct-child Release B15 commit contains only the two release
   packages, `CURRENT_CONTRACT.json`, and the Work 0002 release implementation
   report.
...
| Overall maximum status | `READY_FOR_CONTROLLED_SANDBOX_VALIDATION` |  |
