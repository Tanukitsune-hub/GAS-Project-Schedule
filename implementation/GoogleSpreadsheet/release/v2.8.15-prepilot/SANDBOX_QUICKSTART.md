...
# 2.8.15-prepilot Sandbox Quickstart
...
There is no active transfer or deployment target. The sequence below is only a
future controlled-validation plan and does not authorize any Workspace action.
...
3. Confirm that `apps-script/` contains exactly 23 `.gs` files and
   `appsscript.json`.
...
7. Before S90, verify only the documented Setup-owned 17ﾃ・ Dashboard system
...
at `READY_FOR_CONTROLLED_SANDBOX_VALIDATION`; real runtime and provider checks
remain separately authorized and `NOT EXECUTED`.
