...
var WorkOsTaskReviewPolicy = (function () {
    ...
    return {
      ...
      pending_changes_json: safe ||
        ...
        : {
          ...
          target_task_id: String(action.target_task_id || ''),
  ...
  function buildDeadlinePatch(options) {
    var settings = options || {};
    var dueDate = settings.dueDate;
    var hasDueDate = dueDate !== null &&
      dueDate !== undefined &&
      dueDate !== '';
    if (!hasDueDate) {
      return {
    ...
    var source = String(settings.source || 'AI').toUpperCase();
    var basis = source === 'MANUAL'
      ? 'MANUAL_CONFIRMED'
      : String(settings.deadlineBasis || '');
    if (!Object.prototype.hasOwnProperty.call(
      WorkOsEnums.DeadlineBasis,
      basis
    ) || basis === 'NONE') {
      throw new WorkOsAppError(
        'E_TASK_DEADLINE_PATCH',
        'TASK_REVIEW_POLICY',
        false,
        '譛滄剞譬ｹ諡縺梧悄髯先律縺ｨ荳閾ｴ縺励∪縺帙ｓ縲・
      );
    }
    return {
      due_date: dueDate,
      deadline_basis: basis,
      suggested_due_date: settings.clearSuggestedDueDate === false
        ? (settings.suggestedDueDate || '')
        : ''
    };
  }
    ...
    if (action.action_type === 'UPDATE_DUE') {
      var dueDate =
        Object.prototype.hasOwnProperty.call(action.changes, 'due_date')
          ...
          : action.deadline;
      changes = buildDeadlinePatch({
        dueDate: dueDate,
        deadlineBasis: action.deadline_basis,
        source: 'AI',
  ...
  function applyClassification(classification, environment) {
    ...
    classification.actions.forEach(function (action, actionIndex) {
      ...
      if (resolution.task.pending_action_type &&
        ...
        conflictReviewTask.pending_changes_json.expected_target_row_version =
          Number(resolution.task.row_version);
        conflictReviewTask.pending_changes_json.target_resolution =
          'CONFLICT';
        conflictReviewTask.pending_changes_json.manual_conflicts = conflicts;
        conflictReviewTask.pending_changes_json.current_values = {};
        Object.keys(changes).forEach(function (field) {
          if (Object.prototype.hasOwnProperty.call(
            resolution.task,
            field
          )) {
            conflictReviewTask.pending_changes_json.current_values[field] =
              resolution.task[field];
          }
        });
  ...
  return Object.freeze({
    ...
    buildDeadlinePatch: buildDeadlinePatch,
