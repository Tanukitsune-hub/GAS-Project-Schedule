...
var WorkOsGmailGateway = (function () {
  ...
  function listManualCandidates(options) {
    ...
    for (var summaryIndex = 0;
      ...
      // provider timestamp and Message ID, then retain only the root ID and
      // the latest target plus two immediately preceding references.
      var orderedMessages = sortMessages(messages);
      var first = orderedMessages[0];
      var recent = orderedMessages.slice(
        Math.max(0, orderedMessages.length - 3)
      );
      var latest = recent[recent.length - 1];
      var systemLabels = {};
      (latest.labelIds || []).forEach(function (labelId) {
      ...
      candidates.push({
        ...
        thread_id: String(thread.id || summary.id || latest.threadId || ''),
        stable_thread_key: makeStableThreadKey(first.id, thread.id || summary.id),
        received_at: new Date(messageTimestamp(latest)),
        source_mode: 'MANUAL',
