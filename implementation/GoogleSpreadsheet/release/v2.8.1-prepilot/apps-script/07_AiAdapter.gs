...
var WorkOsAiAdapter = (function () {
  ...
  function baseAction(actionType, options) {
    ...
    return {
      ...
      deadline: value.deadline …3230 tokens truncated…figured: metadataToken(config.model),
    ...
    PROVENANCE_FIELDS.forEach(function (field) {
      if (!metadataToken(value[field])) {
        throw new WorkOsAppError(
          ...
          'AI_VALIDATION',
          false,
          'AI provenance縺悟ｮ牙・縺ｪ隴伜挨蟄舌〒縺ｯ縺ゅｊ縺ｾ縺帙ｓ縲・
        );
      }
    });
    return {
  ...
  function createAdapter(options) {
    ...
    if (String(settings.mode || '').toUpperCase() === 'EXTERNAL') {
  ...
  function getMetadata(adapter) {
    if (adapter && typeof adapter.getMetadata === 'function') {
      return validateProvenance(adapter.getMetadata());
    }
  ...
  function classificationHash(classification, provenance) {
    ...
    var metadata = validateProvenance(provenance || mockMetadata());
