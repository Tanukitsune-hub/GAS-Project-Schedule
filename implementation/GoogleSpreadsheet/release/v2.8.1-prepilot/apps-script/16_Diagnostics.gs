  ...
  function scanLogicalRows(
    ...
    if (!sheet) {
      throw new WorkOsAppError(
        ...
        'DIAGNOSTIC',
        false,
        'Recovery險ｺ譁ｭ縺ｫ蠢・ｦ√↑Sheet縺後≠繧翫∪縺帙ｓ縲・
      );
    }
    ...
    if (sheet.getMaxColumns() !== ids.length) {
      throw new WorkOsAppError(
        ...
        'DIAGNOSTIC',
        false,
        'Recovery險ｺ譁ｭ蟇ｾ雎｡Sheet縺ｮ蛻玲焚縺御ｸ閾ｴ縺励∪縺帙ｓ縲・
      );
    }
    ...
    if (JSON.stringify(actualIds) !== JSON.stringify(ids)) {
      throw new WorkOsAppError(
        ...
        'DIAGNOSTIC',
        false,
        'Recovery險ｺ譁ｭ蟇ｾ雎｡Sheet縺ｮ蜀・Κ蛻悠D縺御ｸ閾ｴ縺励∪縺帙ｓ縲・
      );
    }
...
…2004 tokens truncated…pletedForVersion.indexOf('S70_STORE_PROPERTIES') !== -1;
    ...
    var aiHealth = typeof WorkOsAiAdapter !== 'undefined'
      ? WorkOsAiAdapter.createAdapter().healthCheck()
      : {
        provider: WorkOsConfig.AI_PROVIDER,
        model: WorkOsConfig.AI_PROVIDER === 'MOCK'
          ? WorkOsConfig.MOCK_AI_MODEL
          : '',
        prompt_version: WorkOsConfig.AI_PROVIDER === 'MOCK'
          ? WorkOsConfig.MOCK_PROMPT_VERSION
          : '',
        status: 'MODULE_UNAVAILABLE',
        code: 'E_AI_MODULE_UNAVAILABLE',
        credential_configured: false,
        external_request: false
      };
    checks.push(check(
      'AI_ADAPTER_CONFIGURATION',
      aiHealth.status === 'READY' ? 'PASS' : 'WARN',
      aiHealth.status === 'READY'
        ? ''
        : 'AI Adapter縺ｮlocal險ｭ螳壹′譛ｪ螳御ｺ・〒縺吶ょ､夜Κ騾壻ｿ｡縺ｯ陦後▲縺ｦ縺・∪縺帙ｓ縲・,
      {
        provider: String(aiHealth.provider || ''),
        model: String(aiHealth.model || ''),
        prompt_version: String(aiHealth.prompt_version || ''),
        credential_configured: aiHealth.credential_configured === true,
        health_status: String(aiHealth.status || 'UNKNOWN'),
        health_code: String(aiHealth.code || ''),
