...
# 2.8.11-prepilot Sandbox Quickstart
...
- Automation: `OFF`
- Highest local status: `PHASE8B_SANDBOX_NO_GO_T1_01_SUMMARY`
...
For the existing completed T10 Sandbox, do not follow the generic Setup
sequence below. Use the dedicated T11 transfer guide; Setup, Dashboard refresh,
and all resource-changing operations remain prohibited for the T1-01 summary
retest.
...
7. Before S90, verify only the documented Setup-owned 17×3 Dashboard system
   block may be normalized to the canonical plain-text number-format contract.
   Setup must flush the write, reacquire a fresh Range, and verify the strict
   postcondition before S90 can complete. Stop on any foreign content,
   ownership/control-plane issue, module-contract skew, unverified
   postcondition, or other surface conflict; neither Diagnostic path may
   repair it.
...
at `PHASE8B_SANDBOX_NO_GO_T1_01_SUMMARY` until remote publication and fresh-clone proof.
