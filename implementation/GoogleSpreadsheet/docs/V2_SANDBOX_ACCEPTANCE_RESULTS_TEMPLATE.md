...
or checklist for the current Code `2.8.6-prepilot` / Schema `2.6` / AI Schema
...
new result template may be used only from a separately verified 2.8.6 transfer
envelope after its explicit approval. Do not copy any historical version values
into a current result record.
