...
Current contract: Code `2.8.17-prepilot` / Schema `2.6` / AI Schema `2.0` / Migration `3`
...
1. Accept only completed Gemini `thought* model_output` responses while
   ignoring opaque thought signatures and summaries.
2. Keep exactly one final text output and existing strict classification
   validation; reject unexpected, missing, multiple, or malformed steps.
3. Preserve the Work 0029 callable, Automation-OFF, exact-fixture,
   credential, one-call, UTF-8/history, and no-fallback contracts.
4. Generate Code `2.8.17-prepilot` packages as direct-child A17/B17 with the
...
The highest permitted Work 0030 status is
